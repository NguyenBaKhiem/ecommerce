...
const extractDevices = (ctx, userAgent = DEFAULT_USER_AGENT) => {
  let mobile = null
  let ios = null
  let android = null

  if (userAgent === 'Amazon CloudFront') {
    if (ctx.req.headers['cloudfront-is-mobile-viewer'] === 'true') mobile = true
  } else if (ctx.req && ctx.req.headers['cf-device-type']) { // Cloudflare
    if (ctx.req.headers['cf-device-type'] === 'mobile') mobile = true
  } else {
  ...
  return { mobile, isCrawler, ios, android }
}
...
export default (ctx, inject) => {
  ...
  const { mobile } = extractDevices(ctx, userAgent)
